...
async def get_current_user(user: HTTPAuthorizationCredentials = Depends(bearer_scheme),db: AsyncSession = Depends(create_session)) -> Users:
    payload = decode_access_token(user.credentials)
